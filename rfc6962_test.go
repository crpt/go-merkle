...
func TestRFC6962Hasher(t *testing.T) {
	...
	_, leafHashTrail := trailsFromByteSlices(hashFn, [][]byte{[]byte("L123456")})
	leafHash := leafHashTrail.Hash
	_, leafHashTrail = trailsFromByteSlices(hashFn, [][]byte{{}})
	emptyLeafHash := leafHashTrail.Hash
	_, emptyHashTrail := trailsFromByteSlices(hashFn, [][]byte{})
	...
	for _, tc := range []struct {
		...
		// echo -n '' | sha256sum
		{
			...
			want: "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"[:size*2],
			got:  emptyTreeHash,
		},
		...
		// echo -n 00 | xxd -r -p | sha256sum
		{
			...
			want: "6e340b9cffb37a989ca544e6bb780a2c78901d3fb33738768511a30617afa01d"[:size*2],
			got:  emptyLeafHash,
		},
		// echo -n 004C313233343536 | xxd -r -p | sha256sum
		{
			desc: "RFC6962 Leaf",
			want: "395aa064aa4c29f7010acfe3f25db9485bbd4b91897b6ad7ad547639252b4d56"[:size*2],
			got:  leafHash,
		},
		// echo -n 014E3132334E343536 | xxd -r -p | sha256sum
		{
			desc: "RFC6962 Node",
			want: "aa217fe888e47007fa15edab33c2b492a722cb106c64667fc2b044444de66bbb"[:size*2],
			got:  innerHash(hashFn, []byte("N123"), []byte("N456")),
		},
	} {
		tc := tc
...
func TestRFC6962HasherCollisions(t *testing.T) {
	...
	_, leafHashTrail := trailsFromByteSlices(hashFn, [][]byte{leaf1})
	hash1 := leafHashTrail.Hash
	_, leafHashTrail = trailsFromByteSlices(hashFn, [][]byte{leaf2})
	hash2 := leafHashTrail.Hash
	if bytes.Equal(hash1, hash2) {
		t.Errorf("leaf hashes should differ, but both are %x", hash1)
	}
	// Compute an intermediate subtree hash.
	_, subHash1Trail := trailsFromByteSlices(hashFn, [][]byte{hash1, hash2})
	...
	_, forgedHashTrail := trailsFromByteSlices(hashFn, [][]byte{preimage})
	...
	_, subHash2Trail := trailsFromByteSlices(hashFn, [][]byte{hash2, hash1})
